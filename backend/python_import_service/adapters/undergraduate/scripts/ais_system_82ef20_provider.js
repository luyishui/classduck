// Source: 参考文件/aishedule-master/青果教务/郑州升达经贸管理学院/provider.js
...
//console.log(window.frames["frmDesk"].document.getElementById("frame_1").contentWindow.document)
    ...
    try
    {
        ...
        try{
            console.log("err")
            let info = JSON.parse((await request("get",'gbk',"/jw/common/showYearTerm.action")));
            ht = await request("get",'gbk',"/student/wsxk.xskcb10319.jsp?params="+base64Encode("xn="+info.xn+"&xq="+info.xqM+"&xh="+info.userCode));
            id = "二维表"
        }catch(e){
             console.error(e)
...
async function scheduleTimer() {
  // 内嵌loadTool工具，传入工具名即可引用公共工具函数(暂未确定公共函数，后续会开放)
   ...
   updateText()
    let time = (await AIScheduleSelect({
        titleText:'选择时间',
        contentText:'请选择你要使用的时间',
        selectList :[
          'TIME1:8:10-20:30',
          'TIME2:8:30-20:50'
        ],
  ...
  return {
    ...
    sections:  time=='TIME1'? getTimes(time1):getTimes(time2)// 课程时间表，注意：总长度要和上边配置的节数加和对齐
  }
