// Source: 参考文件/aishedule-master/青果教务/山东女子学院/provider.js

/**
 * @Author: xiaoxiao
 * @Date: 2021-11-10 20:00:51
 * @LastEditTime: 2022-03-03 22:37:26
 * @LastEditors: xiaoxiao
 * @Description: 
 * @FilePath: \AISchedule\青果教务\山东女子学院\provider.js
 * @QQ：357914968
 */
...
try
{
    ...
    if (dom.document.getElementById("xnxq")&&!dom.frames["frame_1"]) {
        ...
        let html = await request("post", 'utf8', "/sdnzjw/frame/desk/showLessonScheduleInfosV14.action?xn=" + xnxq[0] + "&xq=" + xnxq[1])
    ...
    } else if (dom.frames["frame_1"]&&dom.frames["frame_1"].contentDocument.getElementById("cxfs_ewb")!=undefined) {
        ...
        let info = JSON.parse((await request("get",'gbk',"/sdnzjw/jw/common/showYearTerm.action")));
        let courseTable = await request("get",'gbk',"/sdnzjw/student/wsxk.xskcb10319.jsp?params="+base64Encode("xn="+info.xn+"&xq="+info.xqM+"&xh="+info.userCode));
    ...
    for (i = 0; i < tables.length; i++) {
        ...
        try{
            console.log("err")
            let info = JSON.parse((await request("post",'gbk',"/sdnzjw/jw/common/showYearTerm.action")));
            ht = await request("get",'gbk',"/sdnzjw/student/wsxk.xskcb10319.jsp?params="+base64Encode("xn="+info.xn+"&xq="+info.xqM+"&xh="+info.userCode));
            id = "二维表"
        }catch(e){
             console.error(e)
...
function scheduleHtmlParser(html) {
    ...
    if (bz == "index") {
        ...
        result.length==0
        result.push({name:"遇到错误,请加群:628325112,找开发者进行反馈",teacher:"开发者-萧萧",position:bz,day:1,weeks:[1],sections:[{section:1},{section:2}]})
    }
    if(result.length==0) result.push({name:"遇到错误,请加群:628325112,找开发者进行反馈",teacher:"开发者-萧萧",position:"未获取到课表",day:1,weeks:[1],sections:[{section:1},{section:2}]})
...
async function scheduleTimer() {
  ...
  return {
    ...
    sections:  getTimes({
        ...
        lunchTime: {begin: 4, time: 1 * 60 + 40},
