// Source: 参考文件/aishedule-master/强智教务/iframe强智/成都工业学院/provider.js
...
function request(tag, url, data) {
  ...
  function scheduleHtmlProvider(
    ...
    try {
      ...
      let data = "kbjcmsid="
      let html=''
      if(window.frames["Frame0"]!=undefined){
         let dom = window.frames["Frame0"].contentWindow.document
         let data_value = dom.getElementById("kbjcmsid_ul").getElementsByClassName("layui-this")[0].getAttribute("data-value")
         data +=data_value
         html = request("post", "/jsxsd/xskb/xskb_list.do", data);
      }
      else{
          html = request("get", "/jsxsd/xskb/xskb_list.do");
      }
