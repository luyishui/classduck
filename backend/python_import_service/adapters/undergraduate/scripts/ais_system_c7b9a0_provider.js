// Source: 参考文件/aishedule-master/金智教务/长春汽车工业高等专科学校/Provider.js
...
function scheduleHtmlParser(html) {
    ...
    try{
        trs.each(function (inde, em) {
            tds = $(this).find("td[data-role=item]")
            tds.each(function (ind, emmm) {
                div = $(this).find('.mtt_arrange_item')
                div.each(function (indexx, eem) {
                    ...
                    re = {weeks: [], sections: []}
                    namet = $(this).find('.mtt_item_kcmc').eq(0).text()
                    let name = namet.match(/(?<=(([A-Z]|\d)*\s)).*?(?=\[)/)
                    if (name == null) name = namet.match(/(?<=^).*?(?=\[|$)/)
                    re.name = name[0]
                    re.teacher = $(this).find('.mtt_item_jxbmc').eq(0).text()
                    jskc = $(this).find('.mtt_item_room').eq(0).text()
                    jskcar = jskc.split(",")
