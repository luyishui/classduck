// Source: 参考文件/aishedule-master/青果教务/烟台南山学院/provider.js
...
try
{
    ...
    for (i = 0; i < tables.length; i++) {
        ...
        try{
            console.log("err")
            let info = JSON.parse((await request("post",'gbk',"/jw/common/showYearTerm.action")));
...
async function rquestUnit(tag, encod, url, data) {
  ...
  async function scheduleTimer() {
    ...
    try {
      let info = JSON.parse(
        await rquestUnit('post', 'gbk', '/jw/common/showYearTerm.action')
      )
  
      let infos = await rquestUnit(
        'post',
        'gbk',
        '/public/SchoolTimetable.show.jsp?random=' + Math.random(),
        'xn=' +
          info.xn +
          '&xq_m=' +
          info.xqM +
