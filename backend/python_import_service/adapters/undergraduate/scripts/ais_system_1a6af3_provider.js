// Source: 参考文件/aishedule-master/青果教务/昆明医科大学/provider.js
...
async function scheduleHtmlProvider (
  ...
  if(window.location.href.search("homes.html")!==-1){
    await loadTool('AIScheduleTools')
      let index = (await AIScheduleSelect({
        titleText: '课表(用于解决无法跳转到课表页)',
        contentText: '请选择课表类型',
        selectList: ['-1:我已在课表页面','0:教学安排', '1:班级课表'],
      })).split(":")[0]


    let urls = ['/frame/jw/teacherstudentmenu.jsp?menucode=S203','/kbbp/dykb.bjkb.html?menucode=SB03']
    if(index!=='-1'){
...
async function scheduleTimer () {
  ...
  try {
    ...
    times=times.filter(v=>!!v.startTime)
    // console.log(times)
    // console.log(trs)
    return {
