// Source: 参考文件/aishedule-master/青果教务/河西学院/provider.js
...
//console.log(window.frames["frmDesk"].document.getElementById("frame_1").contentWindow.document)
    ...
    try
    {
        ...
        if (dom.document.getElementById("xnxq")&&dom.document.getElementById("frame_1")==undefined) {
            let xnxq = dom.document.getElementById("xnxq").value.split("-")
            let html = await request("post", 'utf8', "/hxxyjw/frame/desk/showLessonScheduleInfosV14.action?xn=" + xnxq[0] + "&xq=" + xnxq[1])
        ...
        } else if (dom.document.getElementById("frame_1")!=undefined && dom.document.getElementById("frame_1").contentWindow.document.getElementById("cxfs_ewb")!=undefined) {
                kbdom = dom.document.getElementById("frame_1").contentWindow.frames["frmReport"].document
                dom1 = dom.document.getElementById("frame_1").contentWindow.document
                let lb = dom1.getElementById("cxfs_lb").checked
                let ewb = dom1.getElementById("cxfs_ewb").checked
                if (lb) id = "列表"
                else if (ewb) id = "二维表"
        }else{
            let info = JSON.parse((await request("get",'gbk',"/hxxyjw/jw/common/showYearTerm.action")));
            let courseTable = await request("get",'gbk',"/hxxyjw/student/wsxk.xskcb10319.jsp?params="+base64Encode("xn="+info.xn+"&xq="+info.xqM+"&xh="+info.userCode));
            kbdom = new DOMParser().parseFromString(courseTable,"text/html")
            id = "二维表"
        }
        /**
         * domm = dom.frames["frmReport"].document
        ...
        try{
            let info = JSON.parse((await request("get",'gbk',"/hxxyjw/jw/common/showYearTerm.action")));
            ht = await request("get",'gbk',"/hxxyjw/student/wsxk.xskcb10319.jsp?params="+base64Encode("xn="+info.xn+"&xq="+info.xqM+"&xh="+info.userCode));
            id = "二维表"
        }catch(e){
             console.error(e)
...
//         return vv && vv.trim()
//     }).filter(v => {
//         return v.search(/长清/) == -1
//     })
//     re.name = ddkcar[0]
//     re.teacher = ddkcar[1]
//     ddkcar = ddkcar.slice(2)
