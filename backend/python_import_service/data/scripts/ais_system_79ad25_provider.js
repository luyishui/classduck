// Source: 参考文件/aishedule-master/树维/内蒙古财经大学/provider.js

/**
 * @Author: xiaoxiao
 * @Date: 2022-02-24 16:46:44
 * @LastEditTime: 2022-03-11 19:46:49
 * @LastEditors: xiaoxiao
 * @Description: 树维教务系统解析
 * @FilePath: \AISchedule\树维\内蒙古财经大学\provider.js
 * @QQ：357914968
 */
...
async function doLogin(sha, aes,dom, prul,urls) {
    ...
    let logRe = await request("post",data,prul + urls.login)
    console.log(logRe)
...
async function getSemestersId(preUrl, courseTableCon) {
    ...
    let data = "tagId="+"semesterBar" + courseTableCon.match(/(?<=semesterBar).*?(?=Semester)/)[0] + "Semester"
    + "&dataType=semesterCalendar&value="
    + courseTableCon.match(/(?<=value:").*?(?=")/)[0]
    + "&empty=false"

    let currentYear = new Date().getFullYear();
    let semesters = eval("(" + await request("post", data, xqurl) + ")").semesters;
    let count = 0;
...
async function getSjarr(preUrl) {
    sleep(0.35)
    let idurl = preUrl + '/courseTableForStd!innerIndex.action';
    ...
    while (courseArr.length <= 1 && i >= 0) {
        ...
        + "&setting.kind=std"
        + "&startWeek="
        + "&semester.id=" + semIdsJson.semesterIds[i]
        + "&ids=" + ids;
        let url = preUrl + "/courseTableForStd!courseTable.action";
        courseArr = (await request("post", data1, url)).split(/var teachers = \[.*?\];/);
...
function scheduleHtmlParser(html) {
    ...
    jss.forEach(v =>{
        ...
        re.name = v.courseName.replace(/\([\w\d\.]{0,}\)/,"");
        re.position = v.roomName.replace(/\(.*?校区\)/g,"");
        re.day = v.day
...
async function scheduleTimer() {
  ...
  return {
    ...
    sections:  getTimes({
        ...
        abnormalClassTime:[{begin:10,time:70}]
