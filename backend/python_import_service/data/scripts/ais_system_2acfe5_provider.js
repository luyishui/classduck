// Source: 参考文件/aishedule-master/树维/重庆科技学院/provider.js
 ...
 * @return {*}
 */
async function doLogin(meth, pri, dom, prul, urls,) {
  let username = ''
  let pas = ''
  ...
  if (!!errtext.length) {
    await AIScheduleAlert({
      contentText: errtext[0].innerText + '>>>请退出重新进入或通过智慧校园直接进行登录<<<',
      titleText: '错误',
      confirmText: '确认',
    })
    return ''
...
async function getSemestersId(preUrl, courseTableCon) {
  ...
  let temp = await request('post', data, xqurl, '加载学期中')
  let semesters = eval(
    '(' + temp.replace(/<script.*?\/script>|\n/gs,"")+ ')'
  ).semesters
  let count = 0
...
async function getSjarr(preUrl) {
  ...
  while (courseArr.length <= 1 && i >= 0) {
     ...
     * 版本二
     */
    courseArr = (await request("post", data1, url)).split(/activity = new /);
    i--
  }
  return courseArr
}
...
async function scheduleHtmlProvider(
  ...
  if(!dom.getElementById("to-index")&&!!document.getElementsByClassName("login_box").length){
      let td = document.createElement("div")
      td.setAttribute("id","to-index")
      let doms = document.createElement("a");
      doms.setAttribute("href","http://web.cqust.edu.cn/");
      doms.innerText="点我去智慧门户\n【请先从智慧门户登录\n然后跳转到教务系统\n再次点击一键导入】";
      doms.style="font-size:20px;color:red;";
      td.appendChild(doms)
      document.getElementsByClassName("login_box")[0].appendChild(td);
  }
  ...
  let warning = `
  >>>导入流程<<<
  1、直接点击一键按导入，在对话框输入账号密码
  2、大概需要等待5秒左右，导入完成后会自动跳转
  ...
  try {
    ...
    let logintag = true;
    // alert(homeText.toString())
    if(typeof homeText == 'number' || typeof homeText == 'object'){
      logintag = false;
    }
    let homeDom = dom
    ...
    if (location.href.search('cas/login') != -1) {
    ...
    } else if (logintag && homeText.search("logout.action") != -1) {
      ...
      arr = await doLogin(null, null, homeDom, preUrl, urls)
      if (!arr) {
        return 'do not continue'
      }
    }
...
function scheduleHtmlParser(html) {
  ...
  //return { courseInfos: result,sectionTimes:djsects }
  return result
}
...
/**
 * @Author: xiaoxiao
 * @Date: 2022-03-30 18:53:53
 * @LastEditTime: 2022-03-30 19:04:30
 * @LastEditors: xiaoxiao
 * @Description: 
...
//   await someAsyncFunc()
  ...
  return {
    ...
    sections:    getTimes({
        ...
        longRestingTime: 20,
        shortRestingTime: 10,
        longRestingTimeBegin: [2,6,10],
        lunchTime: {begin: 4, time: 1 * 60 + 50},
        dinnerTime: {begin: 8, time: 20},
     
      //  abnormalRestingTime: [{begin: 12, time: 10}]
