// Source: 参考文件/aishedule-master/自研教务/河南中医药大学/研究生/provider.js
 ...
 */
 async function scheduleTimer() {
  let sw = window.frames['fnode24'].document.getElementById("ContentPanel3_dgListSw")
  let xw = window.frames['fnode24'].document.getElementById("ContentPanel3_dgListXw")
  let ws = window.frames['fnode24'].document.getElementById("ContentPanel3_dgListWs")
  let sws = sw.outerText.split("\n").filter(v=>v&&v.trim())
  let xws = xw.outerText.split("\n").filter(v=>v&&v.trim())
  let wss = ws.outerText.split("\n").filter(v=>v&&v.trim())
  ...
  //    // let sections = []
      let timeJson = {
        ...
        forenoon: sws.length, // 上午课程节数：[1, 10]之间的整数
        afternoon: xws.length, // 下午课程节数：[0, 10]之间的整数
        night: wss.length, // 晚间课程节数：[0, 10]之间的整数
        sections: []
    }
    let sec = 1;
    let times = [...sws,...xws,...wss]
    times.forEach(element => {
          timeJson.sections.push({
            "section": sec++,
            "startTime": element.trim().split("-")[0],
            "endTime":element.trim().split("-")[1]
          })
    });
