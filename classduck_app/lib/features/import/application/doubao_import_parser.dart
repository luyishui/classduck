...
class DoubaoImportParser {
  static const String _defaultTableName = '豆包导入课表';
  ...
  static ImportTable parse(
    ...
    if (courses.isEmpty) {
      throw const FormatException('未识别到可导入课程，请确认豆包只返回课程 JSON。');
    }
  ...
  static Map<String, dynamic> _normalizeRoot(dynamic decoded) {
    ...
    if (decoded is! Map) {
      throw const FormatException('豆包返回内容不是 JSON 对象或数组。');
    }
  ...
  static List<dynamic> _extractCourses(Map<String, dynamic> root) {
    ...
    throw const FormatException('未找到 courses 数组，请确认提示词要求豆包只返回 JSON。');
  }
  ...
  static ImportCourse _parseCourse(Map<String, dynamic> raw) {
    final String name =
        (_pickString(raw, const <String>[
            ...
            .trim();
    if (name.isEmpty) {
      throw const FormatException('存在课程缺少名称，请重新生成豆包结果。');
    }
  ...
  static String _extractJsonPayload(String source) {
    ...
    if (trimmed.isEmpty) {
      throw const FormatException('请先粘贴豆包返回结果。');
    }
    ...
    throw const FormatException('未找到可解析的 JSON，请确认复制的是豆包原始结果。');
  }
  ...
  static List<int> _parseWeeks(dynamic value) {
    ...
    for (final String rawSegment in normalized.split(',')) {
      ...
      final String segment = rawSegment.replaceAll(RegExp(r'[^0-9\-]'), '');
      if (segment.isEmpty) {
        continue;
      }
