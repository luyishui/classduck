...
async function getKbtable2 () {
  let info = JSON.parse(
    await request('get', 'gbk', '/zjkjjw/jw/common/showYearTerm.action')
  )
  let courseTable = await request(
    'get',
    'gbk',
    '/zjkjjw/student/wsxk.xskcb10319.jsp?params=' +
    base64Encode(
  ...
  try {
    ...
    if (dom.document.getElementById('xnxq') && !dom.frames['frame_1'] && !dom.frames['frmReport']) {
      ...
      let html = await request(
        'post',
        'utf8',
        '/zjkjjw/frame/desk/showLessonScheduleInfosV14.action?xn=' +
        xnxq[0] +
        '&xq=' +
        xnxq[1]
      )
