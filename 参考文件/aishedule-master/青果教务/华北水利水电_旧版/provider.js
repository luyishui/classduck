...
//console.log(window.frames["frmDesk"].document.getElementById("frame_1").contentWindow.document)
    ...
    try
    {
        ...
        if (dom.document.getElementById("xnxq")&&dom.frames["frmReport"]==undefined) {
            ...
            let html = await request("post", 'utf8', "/hsjw/frame/desk/showLessonScheduleInfosV14.action?xn=" + xnxq[0] + "&xq=" + xnxq[1])
            ...
            console.log("req")
            let info = JSON.parse((await request("get",'gbk',"/hsjw/jw/common/showYearTerm.action")));
            let courseTable = await request("get",'gbk',"/hsjw/student/wsxk.xskcb10319.jsp?params="+base64Encode("xn="+info.xn+"&xq="+info.xqM+"&xh="+info.userCode));
        ...
        try{
            console.log("err")
            let info = JSON.parse((await request("get",'gbk',"/hsjw/jw/common/showYearTerm.action")));
            ht = await request("get",'gbk',"/hsjw/student/wsxk.xskcb10319.jsp?params="+base64Encode("xn="+info.xn+"&xq="+info.xqM+"&xh="+info.userCode));
            id = "二维表"
        }catch(e){
             console.error(e)
