/**
 * @Author: xiaoxiao
 * @Date: 2022-06-10 13:53:47
 * @LastEditTime: 2022-08-20 21:50:19
 * @LastEditors: xiaoxiao
 * @Description:
 * @FilePath: \AISchedule\树维\重庆医科大学\provider copy.js
 * @QQ：357914968
 */
 ...
 * @returns
 */
async function doLogin1(sha, dom, prul, urls) {
...
async function isFx(preUrl) {
  let fxs = await request('get', null, preUrl + '/dataQuery.action')
...
async function getSjarr(preUrl) {
  ...
  while (courseArr.length <= 1 && i >= 0) {
    ...
    let data1 =
      ...
      '&startWeek=' +
      '&semester.id=' +
      semIdsJson.semesterIds[i] +
      '&ids=' +
      ids
    let url = preUrl + '/courseTableForStd!courseTable.action'
    courseArr = (await request('post', data1, url)).split(
      /var teachers = \[.*?\];/
    )
...
async function scheduleHtmlProvider(
  ...
  let urls = {
    home: '/home.action',
    login: '/manageLogin.action',
    loginTableClassName: 'login',
  }
  let courseArr = []
  let arr = []
  try {
    //验证是否登录
    //系统接入了sso，改用登录地址验证是否登录
    let homeText = await request('get', null, preUrl + urls.login)
    console.log(preUrl + urls.login, homeText)
    ...
    if (location.href.search('cas/login') != -1) {
      ...
      arr = await doLogin1(sha, homeDom, preUrl, urls)
      if (!arr) {
        return 'do not continue'
      }
    }
