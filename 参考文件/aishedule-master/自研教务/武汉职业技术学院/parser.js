/**
 * @Author: xiaoxiao
 * @Date: 2022-10-03 18:00:01
 * @LastEditTime: 2022-10-05 14:03:51
 * @LastEditors: xiaoxiao
 * @Description:
 * @FilePath: \AISchedule\自研教务\武汉职业技术学院\parser.js
 * @QQ: 357914968
 */
...
function getSections(cellId, rowSpan) {
  let start = cellId.slice(1)
  let count = Number(rowSpan)
  let res = []
  for (let index = 0; index < count; index++) {
    res.push(Number(start) + index)
  }
  return res
}
  ...
  trs.each(function (_, index) {
    ...
    tds.each(function (__, index) {
      ...
      kc.forEach((con) => {
        console.log(con)
        console.log('---------------------------')
        let cellId = $(this).attr('id').replace('Cell', '')
        let rowSpan = !$(this).attr('rowspan') ? 1 : $(this).attr('rowspan')
        console.log(cellId, rowSpan)
        let singleCon = con
          // .replace(/<a\s.*?>/g, '')
          // .replace(/<\/a>/g, '')
          .split(/\<br\>/)
        ...
        let offset = 0
        if (singleCon.length===5 || singleCon.length ==4){
          offset = 2
        }
        let conInfo = { weeks: [], sections: [] }
        // conInfo.name = singleCon[0+offset]
        let temp = con.match(/openKckb(?:.*?)>(.*?)</)
        conInfo.name = !temp?"":temp[1]
        temp = con.match(/openJskb(?:.*?)>(.*?)</)
        conInfo.teacher = !temp?"":temp[1]
        temp = con.match(/openCrkb(?:.*?)>(.*?)</)
        conInfo.position = !temp?"":temp[1]
        conInfo.day = cellId.slice(0, 1)
        conInfo.sections = getSections(cellId, rowSpan)
        conInfo.weeks = getWeeks(con.match(/[>)]\s(.*?周)/)[1])
        result.push(conInfo)
      })
    })
  })
